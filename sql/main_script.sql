...
CREATE TABLE LOTES_INVENTARIO (
    NUMERO_LOTE  NUMBER(6)   PRIMARY KEY,
    ID_TIENDA    NUMBER(4)   NOT NULL,
...
CREATE TABLE JUGUETES (
    ...
    ID_TEMA               NUMBER(4)     NOT NULL,
    ARCHIVO_INSTRUCCIONES VARCHAR2(255),
    NUMERO_PIEZAS         NUMBER(8),
    ID_SET                NUMBER(5),

    CONSTRAINT CHK_JUGUETE_RANGO_EDAD CHECK (RANGO_EDAD IN ( '0 a 2', '3 a 4', '5 a 6','7 a 8', '9 a 11', '12+', 'ADULTOS')),
    CONSTRAINT CHK_JUGUETE_RANGO_PRECIO CHECK (RANGO_PRECIO IN ('A','B','C','D')),
    CONSTRAINT CHK_JUGUETE_ES_SET CHECK (ES_SET IN (0,1)),
    CONSTRAINT FK_JUGUETE_TEMA FOREIGN KEY (ID_TEMA) REFERENCES TEMAS (ID_TEMA),
    CONSTRAINT FK_JUGUETE_SET FOREIGN KEY (ID_SET) REFERENCES JUGUETES (ID_JUGUETE),
    CONSTRAINT CHK_JUGUETE_PIEZAS_OBLIG CHECK ( ES_SET = 1 OR (ES_SET = 0 AND NUMERO_PIEZAS IS NOT NULL AND NUMERO_PIEZAS > 0)),
    CONSTRAINT CHK_JUGUETE_PIEZAS_POS CHECK (NUMERO_PIEZAS IS NULL OR NUMERO_PIEZAS > 0)
);


CREATE TABLE PRODUCTO_RELACION (
    ID_PROD_1 NUMBER(5) NOT NULL,
    ID_PROD_2 NUMBER(5) NOT NULL,
    CONSTRAINT PK_PRODUCTO_RELACION PRIMARY KEY (ID_PROD_1, ID_PROD_2),
    CONSTRAINT FK_PRODUCTO_RELACION_1 FOREIGN KEY (ID_PROD_1) REFERENCES JUGUETES (ID_JUGUETE),
    CONSTRAINT FK_PRODUCTO_RELACION_2 FOREIGN KEY (ID_PROD_2) REFERENCES JUGUETES (ID_JUGUETE),
    CONSTRAINT CHK_PRODUCTO_RELACION_DISTINTOS CHECK (ID_PROD_1 <> ID_PROD_2)
);
...
CREATE TABLE TEMAS (
    ...
    CONSTRAINT CHK_TEMA_TIPO CHECK (TIPO_TEMA IN ('TEMA','SERIE')),
    CONSTRAINT CHK_TEMA_LIC_EXTERNA CHECK (LICENCIA_EXTERNA IN (0,1)),
    CONSTRAINT FK_TEMA_SERIE FOREIGN KEY (ID_SERIE) REFERENCES TEMAS(ID_TEMA)
);
...
CREATE TABLE LOTES_INVENTARIO (
    ID_TIENDA  NUMBER(4) NOT NULL,
    ID_JUGUETE NUMBER(5) NOT NULL,
    NUM_LOTE   NUMBER(4) NOT NULL,
    CANTIDAD   NUMBER    NOT NULL,
        CONSTRAINT CHK_LOTE_CANTIDAD
            CHECK (CANTIDAD >= 0),
    CONSTRAINT PK_LOTE_INVENTARIO PRIMARY KEY (ID_TIENDA, ID_JUGUETE, NUM_LOTE),
    CONSTRAINT FK_LOTE_TIENDA FOREIGN KEY (ID_TIENDA) REFERENCES TIENDAS (ID_TIENDA),
    CONSTRAINT FK_LOTE_JUGUETE FOREIGN KEY (ID_JUGUETE) REFERENCES JUGUETES (ID_JUGUETE)
);
...
CREATE TABLE DESCUENTOS_INVENTARIO (
    ...
    ID_JUGUETE  NUMBER(5) NOT NULL,
    NUM_LOTE    NUMBER(4) NOT NULL,
    ID_DESC_INV NUMBER(4) NOT NULL,
    FECHA       DATE      NOT NULL,
    CANTIDAD    NUMBER    NOT NULL,
    CONSTRAINT CHK_DESC_INV_CANTIDAD CHECK (CANTIDAD > 0),
    CONSTRAINT PK_DESCUENTO_INVENTARIO PRIMARY KEY (ID_TIENDA, ID_JUGUETE, NUM_LOTE, ID_DESC_INV),
    CONSTRAINT FK_DESC_INV_LOTE FOREIGN KEY (ID_TIENDA, ID_JUGUETE, NUM_LOTE) REFERENCES LOTES_INVENTARIO (ID_TIENDA, ID_JUGUETE, NUM_LOTE)
);
...
CREATE TABLE HISTORICO_PRECIO (
    ...
    CONSTRAINT PK_HISTORICO_PRECIO PRIMARY KEY (ID_JUGUETE, FECHA_INICIO),
    CONSTRAINT FK_HPRECIO_JUGUETE FOREIGN KEY (ID_JUGUETE) REFERENCES JUGUETES (ID_JUGUETE),
    CONSTRAINT CHK_HPRECIO_PRECIO_POS CHECK (PRECIO > 0),
    CONSTRAINT CHK_HPRECIO_INTERVALO CHECK (FECHA_FIN IS NULL OR FECHA_FIN > FECHA_INICIO)
);
